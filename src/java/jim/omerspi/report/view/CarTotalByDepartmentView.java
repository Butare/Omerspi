...
public class CarTotalByDepartmentView extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document,
        ...
        List<Carrequisitionservice> printTotalByDepartment = (List<Carrequisitionservice>) model.get("formData");
        ...
        List<Carrequisitionservice> servedDepartment = (List<Carrequisitionservice>) request.getAttribute("departmentList");

        document.add(new Paragraph("\n"));

        PdfPTable table = new PdfPTable(2);

        PdfPCell departmentNames = new PdfPCell(new Phrase("Department Name", tableBold));
        departmentNames.setHorizontalAlignment(Element.ALIGN_CENTER);
        table.addCell(departmentNames);
        ...
        Integer total = 0;
        Integer totalByDepartment = 0;
        for (int i = 0; i < printTotalByDepartment.size(); i++) {

            table.addCell("\t" + printTotalByDepartment.get(i).getCarrequisition().getEmployee().getDepartment().getDepartmentName());
            totalByDepartment = 0;
            for (int j = 0; j < servedDepartment.size(); j++) {
                if (servedDepartment.get(j).getCarrequisition().getEmployee().getDepartment().getDepartmentName().equals(printTotalByDepartment.get(i).getCarrequisition().getEmployee().getDepartment().getDepartmentName())) {
                    totalByDepartment += servedDepartment.get(j).getCarrequisition().getTotalAmount();
                    total += servedDepartment.get(j).getCarrequisition().getTotalAmount();
                }
            }
            table.addCell("" + totalByDepartment);
        }
